...
export default {
  ...
  rate_limit_global_window_ms: Number(process.env.RATE_LIMIT_GLOBAL_WINDOW_MS || 60000), // 1 minute
  rate_limit_global_max_requests: Number(process.env.RATE_LIMIT_GLOBAL_MAX || 150), // 150 requests per minute

  // ───── AUTH GROUP LIMITING (REGISTRATION, LOGIN, PASSWORD RESET) ──────────────────────────────
  rate_limit_auth_window_ms: Number(process.env.RATE_LIMIT_AUTH_WINDOW_MS || 900000), // 15 minutes
  ...
  rate_limit_refresh_window_ms: Number(process.env.RATE_LIMIT_REFRESH_WINDOW_MS || 60000), // 1 minute
  rate_limit_refresh_max_requests: Number(process.env.RATE_LIMIT_REFRESH_MAX || 30), // 30 requests per minute

  // ───── CONTENT CREATION GROUP LIMITING (POSTS, COMMENTS) ──────────────────────────────
  rate_limit_content_window_ms: Number(process.env.RATE_LIMIT_CONTENT_WINDOW_MS || 60000), // 1 minute
  rate_limit_content_max_requests: Number(process.env.RATE_LIMIT_CONTENT_MAX || 25), // 25 posts/comments per minute
