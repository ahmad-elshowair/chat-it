...
export default class FollowModel {
   ...
   */
  async isFollowing(following_id: string, followed_id: string): Promise<{ is_following: boolean }> {
    if (!following_id || !followed_id) {
      throw new Error('Missing required fields: following_id and followed_id are required');
    }
    ...
    try {
      ...
      `;
      const result: QueryResult<{ is_following: boolean }> = await connection.query(sql, [
        following_id,
        followed_id,
      ]);
      ...
      throw new Error(`Failed to check follow model error: ${(error as Error).message}`, {
        cause: error,
      });
    } finally {
      connection.release();
  ...
  async follow(user_id_following: string, user_id_followed: string): Promise<{ message: string }> {
    ...
    try {
      ...
      throw new Error(`Failed to follow user: ${(error as Error).message}`, { cause: error });
    } finally {
      connection.release();
    }
  }
  ...
  async unFollow(
    ...
    try {
      ...
      throw new Error(`Failed to unfollow user: ${(error as Error).message}`, { cause: error });
    } finally {
      connection.release();
    }
  }
  ...
  async getNumberOfFollowings(user_id: string): Promise<number> {
    ...
    try {
      ...
      throw new Error(`Failed to get number of followings: ${(error as Error).message}`, {
        cause: error,
      });
    } finally {
      connection.release();
  ...
  async getNumberOfFollowers(user_id: string): Promise<number> {
    ...
    try {
      ...
      throw new Error(`Failed to get number of followers: ${(error as Error).message}`, {
        cause: error,
      });
    } finally {
      connection.release();
  ...
  async getFollowings(
    ...
    try {
      ...
      sql += direction === 'next' ? ' ORDER BY f.created_at DESC' : ' ORDER BY f.created_at ASC';
      sql += ` LIMIT $${params.length + 1}`;

      params.push(limit);

      const result: QueryResult<TFollowings> = await connection.query(sql, params);
      ...
      throw new Error(`Failed to get followings: ${(error as Error).message}`, { cause: error });
    } finally {
      connection.release();
    }
  }
  ...
  async getFollowers(
    ...
    try {
      ...
      const result: QueryResult<TFollowers> = await connection.query(sql, params);
      ...
      throw new Error(`Failed to get followers: ${(error as Error).message}`, { cause: error });
    } finally {
      connection.release();
    }
  }
