...
class ReportModel {
   ...
   * Uses explicit SQL strings per target type — no dynamic table/column interpolation.
  ...
  async targetExists(targetType: TargetType, targetId: string): Promise<boolean> {
    ...
    try {
      ...
      switch (targetType) {
        case 'post':
          sql = 'SELECT 1 FROM posts WHERE post_id = $1 LIMIT 1';
          break;
        case 'comment':
          sql = 'SELECT 1 FROM comments WHERE comment_id = $1 LIMIT 1';
          break;
        case 'user':
          sql = 'SELECT 1 FROM users WHERE user_id = $1 LIMIT 1';
          break;
      }
   ...
   * Short-circuits for target_type='user' without opening a DB connection.
   * @param targetType - one of 'post', 'comment', 'user'
  ...
  /**
   * Insert a new report. Uses pool.query() for single-statement execution.
   * DB UNIQUE constraint (23505) bubbles up for duplicate detection.
  ...
  async create(input: TReportInput): Promise<TReport> {
    ...
    `;
    const result: QueryResult<TReport> = await pool.query(sql, [
      input.reporter_id,
      input.target_type,
      input.target_id,
      input.reason,
  ...
  async getById(reportId: string): Promise<TReport | null> {
    const sql = `
      ...
      WHERE report_id = $1
    `;
    const result: QueryResult<TReport> = await pool.query(sql, [reportId]);
    return result.rows[0] ?? null;
  }

  /**
   * List reports with optional filters and offset pagination.
   * Runs COUNT and data queries with separate parameter arrays to avoid mutation issues.
  ...
  async list(
    ...
    try {
      ...
      if (filters.targetType) {
        ...
        filterParams.push(filters.targetType);
      }
      ...
      const dataResult: QueryResult<TReport> = await connection.query(dataSql, dataParams);
  ...
  async countByStatus(): Promise<Record<ReportStatus, number>> {
    const sql = `
      ...
      GROUP BY status
    `;
    const result: QueryResult<{ status: ReportStatus; count: number }> = await pool.query(sql);
