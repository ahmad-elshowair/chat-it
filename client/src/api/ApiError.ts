...
export class ApiError extends Error {
  ...
  isCsrfError() {
    return (
      this.status === 403 &&
      (this.data as any)?.error === "CSRF token mismatch!"
    );
  }
  ...
  isMissingFingerprint() {
    return (
      this.isAuthError() &&
      (this.data?.message?.includes("Missing fingerprint") ||
        this.message?.includes("fingerprint"))
    );
  }

  getValidationErrors() {
    if (this.status === 400 && this.data?.errors) {
      return this.data.errors.map(
        (err: any) => err.msg || err.message || String(err)
      );
    }
    return [this.message];
  }
